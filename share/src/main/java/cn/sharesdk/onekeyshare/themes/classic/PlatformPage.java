...
public abstract class PlatformPage extends OnekeySharePage {
    ...
    private static String PANEL_CANCEL_BUTTON_TEXT = "取消";
    ...
    public void onCreate() {
        //分享面板外面的背景
        activity.getWindow().setBackgroundDrawable(new ColorDrawable(0xb32b313d));
        initAnims();

        RelativeLayout llPage = new RelativeLayout(activity);
        activity.setContentView(llPage);

        //分享面板外的空间被这个TextView填充满
        ...
        llPanel.setPadding(0, 0, 0, (int) convertDpToPx(activity, 20));
        ...
        LinearLayout.LayoutParams panelLayoutParams;

        //分享弹窗头部
        TextView headTV = new TextView(activity);
        headTV.setText("分享给朋友");
        headTV.setTextColor(Color.parseColor("#28313d"));
        headTV.setTextSize(TypedValue.COMPLEX_UNIT_SP, 17);
        headTV.setBackgroundColor(Color.parseColor("#f3f4f6"));
        headTV.setGravity(Gravity.CENTER);
        panelLayoutParams = new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT,
                (int) convertDpToPx(activity, 50));
        llPanel.addView(headTV, panelLayoutParams);
        ...
        panelLayoutParams
                .setMargins((int) convertDpToPx(activity, 20), (int) convertDpToPx(activity, 25),
                        (int) convertDpToPx(activity, 20), (int) convertDpToPx(activity, 25));
        llPanel.addView(mvp, panelLayoutParams);

        IndicatorView vInd = new IndicatorView(activity);
        panelLayoutParams = new LinearLayout.LayoutParams(LayoutParams.MATCH_PARENT,
                adapter.getBottomHeight());
        llPanel.addView(vInd, panelLayoutParams);

        //取消按钮
        TextView cancelTV = new TextView(activity);
        panelLayoutParams = new LinearLayout.LayoutParams((int) convertDpToPx(activity, 160),
                (int) convertDpToPx(activity, 35));
        cancelTV.setText(PANEL_CANCEL_BUTTON_TEXT);
        cancelTV.setGravity(Gravity.CENTER);
        cancelTV.setTextSize(TypedValue.COMPLEX_UNIT_SP, 16);
        cancelTV.setTextColor(Color.parseColor("#656d78"));
        cancelTV.setBackgroundResource(R.drawable.qdd_button_bg);
        ...
        llPanel.addView(cancelTV, panelLayoutParams);
...
//        animShow.setDuration(300);
//        animHide = new ScaleAnimation(1, 0, 1, 0, Animation.RELATIVE_TO_SELF, 0.5f,
//                Animation.RELATIVE_TO_SELF, 0.5f);
//        animHide.setDuration(300);
    }
